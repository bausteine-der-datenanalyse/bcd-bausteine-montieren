...
zip_job <- function(folder, zipfile) {
    zip::zip(zipfile, basename(folder), root = dirname(folder))
}
    ...
    for (job in yaml$`jobs-on-parts`) {
        ...
        if (keys[[1]] == "copy") {
            ...
            zip_job(
                ...
                make_name(file.path(target_folder, job$zipfile), part_folder, name, idx)
            )
